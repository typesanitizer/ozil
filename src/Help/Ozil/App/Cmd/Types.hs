...
data DefaultOptions = DefaultOptions
  ...
  -- ^ Enable intelligent search. For example, we can try to check if we're
  -- inside a stack project. If that's the case, then we look for binaries
  -- that might be generated by stack using "stack exec foo -- --help".
  , _defaultOptionsInputs    :: !CmdInput
  -- ^ Input from commandline.
  , _defaultOptionsDebugMode :: !Bool
  -- ^ Enable overlays useful for debugging.
  } deriving Show
